...
const QrCodeGenerator = () => {
  ...
  const isValidUrl = (string) => {
    const res = string.match(/[a-zA-Z]+(?:\s+[a-zA-Z]+)*/);
    return res !== null;
  };
  ...
  const handleGenerateQrCode = async () => {
    if (isValidUrl(dynamicUrl.trim())) {
      ...
      const trackingUrl = `http://192.168.10.65:5000/api/track?url=${encodeURIComponent(dynamicUrl)}&latitude=${latitude}&longitude=${longitude}`;
  ...
  const handleGenerateAndSave = async () => {
    if (isValidUrl(url.trim())) {
      const { latitude, longitude } = await getUserLocation();
      const trackingUrl = `http://192.168.10.65:5000/api/track?url=${encodeURIComponent(url)}&latitude=${latitude}&longitude=${longitude}`;
  ...
  return (
    <div className="qr-container">
      <h2 className="qr-title">QR Code For Product</h2>
      ...
      {qrData && (
        <div className="qr-result">
          ...
          <div className="qr-download-buttons">
            ...
            <button className="qr-share-btn" onClick={shareQRCode}>
              Share QR Code
            </button>
          </div>
        </div>
